...
### High-Accuracy PPE & Hazard Detection on CPU-Only Edge Hardware

[![Python](https://img.shields.io/badge/Python-3.10+-3776AB?logo=python&logoColor=white)](https://python.org)
[![OpenVINO](https://img.shields.io/badge/OpenVINO-2024.x-0068B5?logo=intel&logoColor=white)](https://docs.openvino.ai)
[![YOLO11](https://img.shields.io/badge/YOLO11-Large-00FFFF?logo=ultralytics)](https://docs.ultralytics.com)

*A proposed pipeline to achieve **>10 FPS** real-time safety detection from YOLO11 Large on 8-core CPUs — no GPU required.*

---

[Problem](#-problem) · [Proposed Solution](#-proposed-solution-architecture) · [Expected Results](#-expected-results) · [Setup](#-setup) · [Pipeline Plan](#-pipeline-plan-deep-dive)
...
├── README.md
├── requirements.txt
├── train.py                        # Step 1: Fine-tuning script
├── quantize_f32_to_int8.py         # Step 2: FP32 → INT8 quantization
├── pipeline/
│   ├── preprocess.py               # Letterbox resize, normalize, transpose
│   ├── postprocess.py              # Confidence filter, NMS, coordinate rescale
│   └── openvino_infer.py           # Step 3: Raw OpenVINO inference
├── async_pipeline/
│   └── producer_consumer.py        # Step 4: Async decode-infer pipeline
├── tracker/
│   └── kalman_tracker.py           # Step 5: Kalman filter + sticky violations
├── multi_stream/
│   ├── stream_manager.py           # Step 6: Multi-stream round-robin
│   └── adaptive_resolution.py      # Step 7: Dynamic resolution scaling
├── alerts/
│   ├── alert_system.py             # Step 8: Violation detection + alerts
│   └── alert_log.json              # Output: logged alerts
├── models/
│   ├── best.pt                     # Trained FP32 weights
│   ├── best_int8.xml               # INT8 OpenVINO IR (model graph)
│   └── best_int8.bin               # INT8 OpenVINO IR (weights)
└── data/
...
| # | Requirement | Planned Approach | Status |
|---|---|---|---|
| 1a | INT8 quantization, mAP drop ≤2% | NNCF PTQ, targeting ~0.5% drop | 🔲 Planned |
| 1b | CPU-optimized runtime | OpenVINO LATENCY mode + AVX2 | 🔲 Planned |
| 1c | Single 1080p frame ≤100ms | Raw OV pipeline, ~100ms expected | 🔲 Planned |
| 2a | Async decode-infer pipeline | Producer-consumer, zero idle cycles | 🔲 Planned |
| 2b | Smart frame sampling ≥10 FPS | Frame-skip + Kalman interpolation | 🔲 Planned |
| 3a | 4 concurrent streams | 9-thread architecture, round-robin | 🔲 Planned |
| 3b | Adaptive resolution | Dynamic 1080p/720p/480p scaling | 🔲 Planned |
| 3c | RAM ≤4 GB | Single model instance, < 1 GB expected | 🔲 Planned |
| 4a | 3-frame detection consistency | 3-consecutive-cycle confirmation rule | 🔲 Planned |
| 4b | Alert latency ≤300ms | ~170ms expected | 🔲 Planned |
...
Built for the **Real-Time YOLO11 Ops-Safety Challenge**

**Domain:** AI · Operational Safety · Edge Computing

---

<div align="center">

*Making industrial workplaces safer — one frame at a time.*
